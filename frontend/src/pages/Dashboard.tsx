...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([productsApi.list(), offersApi.list()])
      .then(([p, o]) => { setProducts(p); setOffers(o); })
  ...
  const offerStats = [
    ...
    { label: 'With Storage', value: offers.filter(o => o.products?.storageId).length, icon: 'battery', tone: '#b58b4c' },
    { label: 'With Wallbox', value: offers.filter(o => o.products?.wallboxId).length, icon: 'plug', tone: '#8a7bb0' },
    { label: 'With Heating', value: offers.filter(o => o.products?.heatingSystemId).length, icon: 'heat', tone: '#c0574a' },
  ];
